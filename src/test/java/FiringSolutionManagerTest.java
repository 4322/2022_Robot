...
public class FiringSolutionManagerTest {
  ...
  public void testCalcNewSolution_insideMiddleTarmac() {
    ...
    // Since this is the midpoint, find the average between the two values for the new firing
    // solution
    double newKickerSpeed = (insideTarmac.getKickerSpeed() + middleTarmac.getKickerSpeed()) / 2;
    double newFlywheelSpeed =
        (insideTarmac.getFlywheelSpeed() + middleTarmac.getFlywheelSpeed()) / 2;
    double newHoodPosition = (insideTarmac.getHoodPosition() + middleTarmac.getHoodPosition()) / 2;
    double newAimingTolerance =
        (insideTarmac.getAimingToleranceDeg() + middleTarmac.getAimingToleranceDeg()) / 2;
    double newDistance = (insideTarmac.getDistance() + middleTarmac.getDistance()) / 2;

    FiringSolution insideMiddleTarmac = new FiringSolution(newKickerSpeed, newFlywheelSpeed,
        newHoodPosition, newAimingTolerance, newDistance);
